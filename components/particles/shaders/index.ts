// Next.js's default toolchain has no GLSL loader, so the runtime copies live
// here as plain template strings — kept byte-for-byte identical to the
// `.glsl` files alongside them, which serve as the documented, lint/highlight
// -friendly source of truth for editing in an editor with shader tooling.
...
void main() {
  ...
  displaced.xy += dir * influence * 1.4;
  displaced.xy += tangent * influence * 1.1;
  displaced.z  += influence * 0.6;
